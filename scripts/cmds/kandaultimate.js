  ...
  onStart: async function ({ api, event, args, message }) {
    ...
    if (command === "on") {
      if (isRandomVideoEnabled) {
        api.sendMessage("corn is already enabled.", event.threadID, event.messageID);
      } else {
        isRandomVideoEnabled = true;
        api.sendMessage("corn is now enabled", event.threadID, event.messageID);
        startSendingRandomVideos(api, event.threadID);
      }
    } else if (command === "off") {
      if (!isRandomVideoEnabled) {
        api.sendMessage("corn is already disabled.", event.threadID, event.messageID);
      } else {
        isRandomVideoEnabled = false;
        clearInterval(intervalId);
        api.sendMessage("corn is now disabled.", event.threadID, event.messageID);
      }
    } else {
